...
import { Search, Plus, Camera, User, Phone, MapPin, Calendar, Trash2, Edit3, MoreHorizontal, X, Save, Shield } from 'lucide-react';
...
export const CameraManagement: React.FC = () => {
  ...
  const handleEdit = (record: CameraAccountRecord) => {
    ...
    setAccountPassword(record.accountPassword || '');
    ...
    <div className="flex flex-col gap-6 p-4 md:p-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Lưu trữ tài khoản xem camera của khách hàng</p>
        </div>
        <button 
          onClick={() => { resetForm(); setShowAddModal(true); }}
          className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 transition-all active:scale-95"
        >
          <Plus size={20} />
          Thêm tài khoản mới
        </button>
        ...
        <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:border-emerald-400 outline-none transition-all shadow-inner"
              value={searchTerm}
                ...
                <tr key={record.id} className="hover:bg-slate-50/80 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center font-bold text-sm border border-emerald-100">
                  ...
                  <td className="px-6 py-4">
                    ...
                    {record.cameraBrand && (
                      <div className="text-[10px] text-emerald-600 font-black uppercase tracking-tighter mt-1 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 inline-block">
                        Hãng: {record.cameraBrand}
                      </div>
                    )}
                    {record.accountPassword && (
                      <div className="text-xs text-slate-500 font-mono mt-1 bg-slate-100 px-2 py-0.5 rounded inline-block ml-1">
                        Pass: {record.accountPassword}
                      </div>
                    )}
                  </td>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                {editingRecord ? <Edit3 size={20} className="text-emerald-500" /> : <Camera size={24} className="text-emerald-500" />}
                {editingRecord ? 'Chỉnh sửa tài khoản' : 'Thêm tài khoản Camera mới'}
              </h2>
              <button 
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                  ...
                  {showSuggestions && customerSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                      {customerSuggestions.map(c => (
                        <div 
                          key={c.id} 
                          className="px-4 py-3 hover:bg-emerald-50 cursor-pointer border-b border-slate-50 last:border-0 flex flex-col gap-0.5"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <Camera size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input 
                      type="text"
                      className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-emerald-400 outline-none text-[15px] font-medium transition-all"
                      placeholder="Hikvision, Dahua, Ezviz..."
                      value={cameraBrand}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text"
                    className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-emerald-400 outline-none text-[15px] font-medium transition-all"
                    placeholder="Địa chỉ lắp đặt camera..."
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider ml-1">Mật khẩu tài khoản</label>
                <div className="relative">
                  <Plus size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input 
                    type="text"
                    className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-emerald-400 outline-none text-[15px] font-mono font-medium transition-all"
              ...
              <div className="space-y-1.5">
                ...
                <textarea 
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-emerald-400 outline-none text-[15px] font-medium transition-all resize-none h-24"
                  placeholder="Loại camera, số kênh, vị trí đầu ghi..."
                  value={note}
            ...
            <div className="p-6 bg-slate-50 flex gap-3">
              ...
              <button 
                onClick={handleSave}
                className="flex-[2] bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 transition-all active:scale-95"
              >
                <Save size={20} />
                Lưu thông tin
              </button>
